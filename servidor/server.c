...
int main( int argc, char *argv[] ) {
   ...
   listen(sockfd,5);
	...
	for(;;) { //Bucle infinito
		...
		for(i = 0; i <= fdmax; i++) {
			if (FD_ISSET(i, &read_fds)) { // Obtenemos una coneccion
				if (i == sockfd) {	// Si es nueva
					clilen = sizeof(cli_addr);
					if ((newsockfd = accept(sockfd, (struct sockaddr *)&cli_addr,&clilen)) == -1) { //Evaluamos la aceptacion
						perror("\nERROR accept");
					} else { //Aceptada!
					...
					if ((nbytes = recv(i, buffer, sizeof(buffer), 0)) <= 0) {// Si existe un error en la recepcion o la conexion se cierra por el cliente
						...
						printf("\nEste es el mensaje: %sLongitud: %i\n",buffer, strlen(buffer)-1);  //Mensaje (buffer)!!!
